...
prior_sim <- function (model, parameter){
  ...
  for (t in 3:length(parameter)) {
    ...
    plot(density(inv_logit_scaled(prior$Intercept + prior[, colnames(prior) == parameter[t]]) - inv_logit_scaled(prior$Intercept),
                 adjust = .01), main = "Condition probability difference")
  }
}
#-----------------------------------------------------------------------------------------------------------------------------------------
...
bayes_samples <- function (model){
  ...
  max_rhat <- round(max(rhat(model), na.rm = T),digit = 3) 
  min_rhat <- round(min (rhat(model), na.rm = T), digit = 3)
  max_sample <- max(model_sample$eff_sample, na.rm = T)
  min_sample <- min(model_sample$eff_sample, na.rm = T)
...
restricted_BF <- function(ordered_model, total_samples, n_conditions, prior_model_count = 1){
  count <- sum(ordered_model)
  prior <- dim(gtools::permutations(n_conditions, n_conditions))[1]
  ordervsunorder<- (count/total_samples) / ((1/prior) * prior_model_count)
