...
Plot_LMM <- function (LMM_model, ref_group, dummy1, dummy2, plot_display, DV){
  ...
  random_slope<- data.frame ('Participant'=rownames(u0_u1) , ref_group = intercept,
                             dummy1 = intercept + u0_u1[[2]], dummy2 = intercept + u0_u1[[3]])
  ...
  if (plot_display == 1){
    plot(random_slope$Participant,u0_u1[[2]], xlab = 'Participants', ylab = paste(dummy1,'condition random slopes')); 
    abline(0,0, col = 'red')
    abline(LMM_model@beta[2], 0, col = 'blue')
  } else if (plot_display == 2){
    plot(random_slope$Participant,u0_u1[[3]], xlab = 'Participants', ylab = paste(dummy2,'condition random slopes'))
    abline(0,0, col = 'red')
    abline(LMM_model@beta[3], 0, col = 'blue')
  } else if (plot_display == 3){
    lattice::xyplot (Mean ~ Condition, data = plot_0v1.df, ylab = DV, groups = Participants, type = c("p","l"), 
                     auto.key = list(space = "left", columns = 4,title = "Participants", cex.title = 1), ylim = c(1, 7))
  } else if (plot_display == 4){
    lattice::xyplot (Mean ~ Condition, data = plot_0v2.df, ylab = DV, groups = Participants, type = c("p","l"), 
                     auto.key = list(space = "left", columns = 4,title = "Participants", cex.title = 1), ylim = c(1, 7))
  }
}
#-------------------------------------------------------------------------------------------------------------------------------------
...
bar_plot <- function (means, palette){
  plot <- ggplot(means, aes(x = Conditions, y = emmean)) +
                 ...
                 xlab ("Conditions") +
                 geom_errorbar(aes(ymin = emmean - SE, ymax = emmean + SE), width = .15, position = osition_dodge(.9)) +
